import pytest
import torch
from lib.Solvers import weight_approx, svd_llm_method
import numpy as np
...
def test_equivalence_weight_approx_and_svd_llm():
    ...
    error = torch.norm(W_approx_our - W_approx_svd_llm, 'fro')
...
def test_svd_llm_gpu():
    ...
    A_gpu, B_gpu = svd_llm_method(W_gpu, X_gpu, r)
...
def test_svd_llm_cpu_gpu_equivalence():
    ...
    A_cpu, B_cpu = svd_llm_method(W, X, r)
    A_gpu, B_gpu = svd_llm_method(W.cuda(), X.cuda(), r)
